...
public class AntlrToLogic extends LogicBaseVisitor<Logic>{
	...
	public Logic visitSingleBool(SingleBoolContext ctx) {
		return new BoolVar(ctx.ID().getText(), new UninitialDecl());
	}
	
	// uninitialized int variable declaration
	@Override
	public Logic visitSingleInt(SingleIntContext ctx) {
		return new IntVar(ctx.ID().getText(), new UninitialDecl());
	}
	...
	public Logic visitBoolValueDecl(BoolValueDeclContext ctx) {
		return new BoolVar(ctx.ID().getText(), visit(ctx.boolExpr()), new InitialDecl());
	}
	
	// initialized int variable declaration
	@Override
	public Logic visitIntValueDecl(IntValueDeclContext ctx) {
		return new IntVar(ctx.ID().getText(), visit(ctx.arithmetic()), new InitialDecl());
	}
