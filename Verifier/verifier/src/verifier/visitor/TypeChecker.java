	...
	public void visitIntVar(IntVar v) {
		...
		// j : INTEGER = i + 2
			...
			if (!(varMap.containsKey(infixPrinter.infixOutput))) {
				...
				errormsg.add("Error: " + infixPrinter.infixOutput + " is not integer type, cannot perform this assignment.");
			}
	...
	public void visitBoolArrayVar(BoolArrayVar a) {
		...
		// verify a[1]
			...
			if (!(varMap.containsKey(infixPrinter.infixOutput))) {
				...
				errormsg.add("Error: " + infixPrinter.infixOutput + " is not integer type, cannot use it as array index value.");
			}
...
//			}
		...
		// e.g. a : ARRAY[BOOLEAN] = << p, q, p and q >>
			...
			for (int i = 0; i < a.elementValue.size(); i++) {
				TypeChecker checker = new TypeChecker();
				a.elementValue.get(i).accept(checker);
				errormsg.addAll(checker.errormsg);
			}
			if (errormsg.isEmpty()) {
				// if this variable is declared for the first time, simply add it to the map
				if (!varMap.containsKey(a.name)) {
					varMap.put(a.name, new Pair<VarType, Verifier>(new BoolArray(), null));
					arrayMap.put(a.name, a.elementValue);
				}
				// if this variable is not declared for the first time, change its type to unknown type
				// and add the error message
				else {
			...
			for (int i = 0; i < a.elementValue.size(); i++) {
				...
				InfixPrinter printer = new InfixPrinter();
				a.elementValue.get(i).accept(printer);
		...
		// e.g. a : ARRAY[INTEGER] = << 1, 2, 6, 0 >>
			...
			for (int i = 0; i < a.elementValue.size(); i++) {
				...
				if (!(varMap.containsKey(printer.infixOutput))) {
					...
					errormsg.add("Error: " + printer.infixOutput + " is not integer type, cannot perform this assignment.");
				}
			}
			if (errormsg.isEmpty()) {
				// if this variable is declared for the first time, simply add it to the map
...
//			}
	...
	public void visitPair(PairVar p) {
			...
			if (errormsg.isEmpty()) {
				...
				// if it's verifying the second element
					...
					// if the element name does not match any of the pair element name
						...
						errormsg.add("Error: Pair " + p.name + " does not contain the element " + p.element + ".");
					}
				}
			}
		}
		...
		// unnamed decl
		// used for method return value - find_max(a : ARRAY[INTEGER]) : PAIR[BOOLEAN;INTEGER]
		// or pair element - p : PAIR[BOOLEAN; INTEGER]
		...
		// p := [true, 1]
			...
			// if assigned both assigned value is type correct
			
			if (errormsg.isEmpty()) {
				if (!varMap.containsKey(p.name)) {
					errormsg.add("Error: variable " + p.name + " has not been declared.");
				}
				else if (parameterMap.containsKey(p.name)) {
					errormsg.add("Error: Variable " + p.name + " is parameter. Cannot perform this assignment.");
				}
				else {
					// type check if the assigned value type is the same as pair element type
					// if assigning int to real type, do nothing
					if (pairVarMap.get(p.name).a instanceof RealVar) {
						if (varMap.get(leftvaluePrinter.infixOutput).a instanceof RealType || varMap.get(leftvaluePrinter.infixOutput).a instanceof IntType) {
					...
					else if (pairVarMap.get(p.name).a instanceof BoolVar && !(varMap.get(leftvaluePrinter.infixOutput).a instanceof BoolType)) {
						errormsg.add("Error: Assigned value " + leftvaluePrinter.infixOutput + " does not have the same type as pair element. "
								+ "Cannot perform this assignment.");
						
					}
					else if (pairVarMap.get(p.name).a instanceof IntVar && !(varMap.get(leftvaluePrinter.infixOutput).a instanceof IntType)) {
						errormsg.add("Error: Assigned value " + leftvaluePrinter.infixOutput + " does not have the same type as pair element. "
								+ "Cannot perform this assignment.");
					}
					else if (pairVarMap.get(p.name).b instanceof BoolVar && !(varMap.get(rightvaluePrinter.infixOutput).a instanceof BoolType)) {
						errormsg.add("Error: Assigned value " + rightvaluePrinter.infixOutput + " does not have the same type as pair element. "
								+ "Cannot perform this assignment.");
						
					}
					else if (pairVarMap.get(p.name).b instanceof IntVar && !(varMap.get(rightvaluePrinter.infixOutput).a instanceof IntType)) {
						errormsg.add("Error: Assigned value " + rightvaluePrinter.infixOutput + " does not have the same type as pair element. "
								+ "Cannot perform this assignment.");
					}
	...
	public void visitAssignment(Assignments a) {
		...
		if (errormsg.isEmpty()) {
			...
			if (a.mode instanceof modes.NormalVarAssign || a.mode instanceof modes.ResultNormalAssign) {
				...
				else if (varMap.get(a.name).a instanceof ArrayType || varMap.get(assignValueprinter.infixOutput).a instanceof ArrayType) {
			...
			// if left hand side is array element
				...
				if (errormsg.isEmpty()) {
					if (!varMap.containsKey(a.name)) {
						...
						errormsg.add("Error: variable " + a.name + " is not declared as an array.");
					}
			...
			// if left hand side is pair element
				...
				else if (!(varMap.get(a.name).a instanceof PairType)) {
					errormsg.add("Error: variable " + a.name + " is not declared as a pair.");
				}
				else if (a.element.equals("first") || a.element.equals(pairVarMap.get(a.name).a.name)) {
					if (pairVarMap.get(a.name).a instanceof RealVar && varMap.get(assignValueprinter.infixOutput).a instanceof IntType) {
						// no error
					}
					else if ((pairVarMap.get(a.name).a instanceof BoolVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof BoolType))
							|| (pairVarMap.get(a.name).a instanceof IntVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof IntType))
							|| (pairVarMap.get(a.name).a instanceof RealVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof RealType))) {
						errormsg.add("Error: variable " + a.name + ".first"  
								+ " does not have the same type as " 
								+ assignValueprinter.infixOutput + ". Cannot perform this assignment.");
					}
				}
				else if (a.element.equals("second") || a.element.equals(pairVarMap.get(a.name).b.name)) {
					if (pairVarMap.get(a.name).b instanceof RealVar && varMap.get(assignValueprinter.infixOutput).a instanceof IntType) {
						// no error
					}
					else if ((pairVarMap.get(a.name).b instanceof BoolVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof BoolType))
							|| (pairVarMap.get(a.name).b instanceof IntVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof IntType))
							|| (pairVarMap.get(a.name).b instanceof RealVar && !(varMap.get(assignValueprinter.infixOutput).a instanceof RealType))) {
						errormsg.add("Error: variable " + a.name + ".second"  
								+ " does not have the same type as " 
								+ assignValueprinter.infixOutput + ". Cannot perform this assignment.");
					}
				}
				else {
					errormsg.add("Error: Pair " + a.name + " does not contain the element " + a.element + ".");
				}
			}
		}
	}
	...
	public void visitResults(Results r) {
		...
		if (errormsg.isEmpty()) {
			...
			if (varMap.get(r.name).a instanceof BoolArray) {
				varMap.put(p.infixOutput, new Pair<VarType, Verifier>(new BoolType(), null));
			}
			else if (varMap.get(r.name).a instanceof IntArray) {
				varMap.put(p.infixOutput, new Pair<VarType, Verifier>(new IntType(), null));
			}
			else if (varMap.get(r.name).a instanceof RealArray) {
				varMap.put(p.infixOutput, new Pair<VarType, Verifier>(new RealType(), null));
			}
		}
	}
	...
	public void visitUnknownVar(UnknownVar n) {
		...
		else if (n.mode instanceof modes.WrongArrayDecl) {
			varMap.put(n.name, new Pair<VarType, Verifier>(new UnknowType(), null));
			errormsg.add("Error: variable " + n.name + " is not array type.");
		}
		else if (n.mode instanceof modes.WrongPairDecl) {
			varMap.put(n.name, new Pair<VarType, Verifier>(new UnknowType(), null));
			errormsg.add("Error: variable " + n.name + " is not pair type.");
		}
		else if (n.mode instanceof modes.IsArrayAssign) {
			varMap.put(n.name, new Pair<VarType, Verifier>(new UnknowType(), null));
			errormsg.add("Error: Array assignment is not allowed.");
		}
		else if (n.mode instanceof modes.IsPairAssign) {
			varMap.put(n.name, new Pair<VarType, Verifier>(new UnknowType(), null));
			errormsg.add("Error: Pair assignment is not allowed.");
		}
