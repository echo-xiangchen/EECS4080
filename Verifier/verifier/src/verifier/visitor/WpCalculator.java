...
import modes.ArithVerification;
import modes.BoolVerification;
import modes.NormalVarAssign;
...
public class WpCalculator implements Visitor{
	...
	public void visitAssignment(Assignments a) {
		...
		else if (a.mode instanceof modes.ArrayEAssign) {
		...
		// if left hand side is result array
		else if (a.mode instanceof ResultArrayAssign) {
			var = new Results(a.index);
		...
		else if (a.mode instanceof modes.PairEAssign) {
			var = new PairVar(a.name, a.element, new BoolVerification());
		...
		// if left hand side is result pair
		else if (a.mode instanceof ResultPairAssign) {
...
//		System.out.println("wp: " + wpPrinter4.infixOutput);
		...
		step5imps.add(new Assignments("variant", ((VariantStat) l.variant).variant.b.copy(), new NormalVarAssign()));
