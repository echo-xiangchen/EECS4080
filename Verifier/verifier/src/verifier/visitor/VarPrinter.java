...
public class VarPrinter implements Visitor {
	...
	public void visitBoolArrayVar(BoolArrayVar a) {
		...
		// e.g. a : ARRAY[BOOLEAN] = << p, q, p and q >>
			...
			String value = "<< ";
			arrayCount.put(a.name, String.valueOf(a.elementValue.size()));
	...
	public void visitRealArrayVar(RealArrayVar a) {
		...
		// e.g. a : ARRAY[REAL] = << 1.5, 2, 6.0, 0 >>
			...
			value = value.substring(0, value.length() - 2) + " >>";
			allVarMap.replace(a.name, new Pair<String, String>("ARRAY[REAL]", value));
			unusedVarMap.replace(a.name, new Pair<String, String>("ARRAY[REAL]", value));
		}
		else if (a.mode instanceof modes.AnonymousDecl) {
			objMap.put(a.name, a);
		}
	...
	public void visitPair(PairVar p) {
		if (p.mode instanceof modes.UninitializedDecl) {
			...
			if (p.left().name != null && p.right().name != null) {
				
				allVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().name + " : " + p.left().typeStr
						+ " ; " + p.right().name + " : " + p.right().typeStr + "]", null)); 
				unusedVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().name + " : " + p.left().typeStr
						+ " ; " + p.right().name + " : " + p.right().typeStr + "]", null));
				
			}
			else {
				allVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().typeStr
						+ " ; " + p.right().typeStr + "]", null)); 
				unusedVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().typeStr
						+ " ; " + p.right().typeStr + "]", null));
			}
			...
			if (p.element.equals("first")) {
				if (unusedVarMap.containsKey(TypeChecker.pairVarMap.get(p.name).a.name)) {
					unusedVarMap.remove(TypeChecker.pairVarMap.get(p.name).a.name);
				}
			}
			else if (p.element.equals("second")) {
				if (unusedVarMap.containsKey(TypeChecker.pairVarMap.get(p.name).b.name)) {
					unusedVarMap.remove(TypeChecker.pairVarMap.get(p.name).b.name);
				}
			}
			else {
			...
			VarPrinter leftvaluePrinter = new VarPrinter();
			VarPrinter rightvaluePrinter = new VarPrinter();
			
			p.leftvalue().accept(leftvaluePrinter);
			p.rightvalue().accept(rightvaluePrinter);
			
			// call infix printer to output the left and right value
			InfixPrinter leftInfixPrinter = new InfixPrinter();
			InfixPrinter rightInfixPrinter = new InfixPrinter();
			
			p.leftvalue().accept(leftInfixPrinter);
			p.rightvalue().accept(rightInfixPrinter);
			
			String value = "[" + leftInfixPrinter.infixOutput + " ; " + rightInfixPrinter.infixOutput + "]";
			
			if (p.left().name != null && p.right().name != null) {
				allVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().name + " : " + p.left().typeStr
						+ " ; " + p.right().name + " : " + p.right().typeStr + "]", value)); 
				unusedVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().name + " : " + p.left().typeStr
						+ " ; " + p.right().name + " : " + p.right().typeStr + "]", value));
			}
			else {
				allVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().typeStr
						+ " ; " + p.right().typeStr + "]", value)); 
				unusedVarMap.put(p.name, new Pair<String, String>("PAIR[" + p.left().typeStr
						+ " ; " + p.right().typeStr + "]", value));
			}
		}
			...
			allVarMap.replace(p.name, new Pair<String, String>(allVarMap.get(p.name).a, value));
		}
